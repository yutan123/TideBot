...
references 固定 3 条、每条不超过 15 字，只给回复方向和情绪基调，不要给具体句子。机器人会根据自己的人设和说话方式自行组织语言，绝不会照抄。例如：表达关心但保持距离、轻松调侃化解尴尬、认真解释但带点小脾气。''';
      ...
      if (statusCode == 200) {
        ...
        if (persistResponse) {
          ...
          if (innerThoughtEnabled && hasInnerThought) {
            final thoughtRow = <String, dynamic>{
              'id': 'msg_t_$ts',
              'bot_id': botId,
              'role': 'assistant',
              'type': 'inner_thought',
  ...
  String _buildSystemPrompt(Map<String, dynamic> bot, String? activeGame) {
    ...
    p +=
        '\n【适时沉默】当 choose_silence 工具可用时，你可在用户明确暂不希望回复、对话自然结束且不需回应、或主动问候不宜打扰时调用它；调用后不要输出文本。不得在求助、悲伤、愤怒、困惑、危机、提问或任何需要回应时调用，绝不可作为惩罚、冷暴力、控制或施压。\n【日记】你拥有 write_diary 工具。遇到值得保留的经历、感受或事件时可按需调用，把日记内容写入工具参数；日记应记录机器人自己的日常、感受和经历，不仅限于与用户的对话，也应包含自己的生活事件。日记内容不要包含日期和时间信息，系统会自动记录。绝不把日记正文当作聊天消息发送。';
    return p;
  }
